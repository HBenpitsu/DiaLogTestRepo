...
- **AI 連携**: Built-In 方式（バックエンドの環境変数 `GEMINI_API_KEY` を使用）。
- **日記作成**: 
...
- ドキュメントサーバの独立プロセス化（バックエンドに内包させる）。
...
### Phase 1: プロジェクト基盤の構築
1. **ディレクトリ構造の決定**: `backend/`, `frontend/`, `proto/` の構成を作成。
...
### Phase 3: フロントエンドのコア実装 (Next.js)
1. **API クライアントの設定**: `connect-es` を用いたバックエンドとの通信設定。
2. **チャット画面 (Diary Chat)**:
    - ストリーミング応答の表示。
    - 入力フォームと「日記を確定する」ボタン。
...
## 3. マイルストーン
- [ ] **Phase 1**: プロトコル定義とバックエンド/フロントエンドの疎通確認。
