# DiaLogCore Architecture

このドキュメントは、ヘキサゴナルアーキテクチャ（Ports & Adapters）に基づき、DiaLogCoreサーバの内部構造と外部連携のメカニズムを定義する。
...
graph TD
    ...
    subgraph DiaLogCore ["2. DiaLogCore Server (Project Scope)"]
        direction TB
...
- **対話の開始と外部情報候補の取得**: 新規の日記対話を開始する際、`IWeatherProvider` および `INewsProvider` を呼び出し、その日の天気と主要ニュースの**候補を事前取得**する。
    - これらの候補はUIServerに提供され、ユーザーが実際に日記に含めるものを選択する。
...
- **実行トリガーの制御**: 運用モード（Cloud/Self-host）に基づき、実行タイミングを制御する。
    - クラウド版: ユーザーによるUIServerからの明示的なリクエスト（手動）を主眼とする。
...
| :--- | :--- | :--- | :--- |
| **クラウド** | `Auth Token` (JWT) | Google SSO (OIDC) 連携。検証後、DiaLogCoreが JWT を発行。 | ユーザー識別 (Multi-tenant) および認可。 |
| **セルフホスト** | `Local Auth Token` | アプリケーション起動時にランダム生成。Cookie 経由でUIServerへ配布。 | **セキュリティ保護 (CSRF/不正アクセス防止)**。識別は不要。 |
...
2.  **プロセス間共有 (Internal)**:
    *   **DocServer → DiaLogCore**: `DOCSERVER_AUTH_TOKEN` 環境変数を介して、DocServer, DiaLogCore プロセスの通信用トークンを共有する.
    *   **DiaLogCore → UIServer**: `DIALOGCORE_AUTH_TOKEN` 環境変数を介して、DiaLogCore, UIServer (Next.js) プロセスの通信用トークンを共有する.
3.  **ブラウザへの配布 (Handshake)**:
    *   ユーザーがブラウザでアクセスした際、UIServer-Client 層は `Set-Cookie` ヘッダーを用いてトークン（`Auth Token`）をブラウザに保持させる。
4.  **リクエストの検証 (API Call)**:
...
- **メカニズム**: `Scheduler` は無効化される。すべての要約生成は、UIServerからの `CreateSummary` RPC 呼び出しをトリガーとする。
